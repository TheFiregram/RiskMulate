...
function discoverScene() {
  ...
  // Prefer an already-exposed scene handle if present.
...
function setFindingVisual(findingId, state) {
  ...
  if (findingId === 'access-obstruction') {
    // Committed control removes the obstructing stack from the service route.
    // Projected control fades it so the student can preview residual access.
...
function selectionState(progress, continuity) {
  ...
  for (const [findingId, actionId] of Object.entries(FINDING_ACTION_MAP)) {
    if (!selected.has(actionId)) {
      states[findingId] = 'off';
    } else if (mode === 'ACTIVE') {
      states[findingId] = 'committed';
    } else if (mode === 'PROJECTED') {
      states[findingId] = 'projected';
    } else {
      states[findingId] = 'off';
    }
  }
...
function applyPlantResponse(progress = {}, continuity = null) {
  ...
  // Flange leak is created in game.js, not field-evidence; re-index occasionally.
  ...
  // Teaching feedback only when the plan actually changes.
  if (!previous) return;
...
export function installPlantResponseEffects(THREE) {
  ...
  // game.js does not currently export the scene; recover it once the canvas exists
  // and also accept an explicit registration from the runtime bridge.
  ...
  // Poll briefly after boot because game.js creates the scene asynchronously relative to bootstrap installers.
  let attempts = 0;
